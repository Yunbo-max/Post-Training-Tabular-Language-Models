...
def main():
    ...
    # ABLATION: Random group selection - split into high/low by median, then randomly pair
    print("ABLATION: Random group pairing - split by median score, randomly pair across groups")
    import random
    random.seed(42)
    half = len(sorted_indices) // 2
    high_indices = list(sorted_indices[:half])  # top 50% by score
    low_indices = list(sorted_indices[half:])   # bottom 50% by score
    random.shuffle(high_indices)  # randomize within high group
    random.shuffle(low_indices)   # randomize within low group
    num_pairs = min(len(high_indices), len(low_indices))
    
    for i in range(num_pairs):
        chosen_idx = high_indices[i]   # random from high group
        rejected_idx = low_indices[i]  # random from low group
