...
if __name__ == '__main__':
    ...
    # # Create filename from the path
    # if args.path:
    #     # Extract meaningful filename from path
    #     path_parts = args.path.replace('.csv', '').split('/')
    #     if 'great.csv' in args.path:
    #         filename = 'baseline_great'
    #     else:
    #         # Find iteration and file info
    #         filename = f"{model}"
    #         for part in path_parts:
    #             if 'dpo_syn_iter' in part:
    #                 filename = part
    #                 break
    #             elif 'iteration_' in part:
    #                 iter_num = part.split('_')[-1]
    #                 filename = f"{model}_iter{iter_num}"
    # else:
    #     filename = model
    ...
    if args.path:
        # Extract meaningful filename from path
        full_path = args.path
        basename = os.path.basename(full_path).replace('.csv', '')
        
        if 'great.csv' in full_path:
            filename = 'baseline_great'
        else:
            # Use the full basename to include iteration AND file number
            # e.g., great_syn_iter_2_1.csv -> great_syn_iter_2_1.txt
            # e.g., great_syn_iter_2_10.csv -> great_syn_iter_2_10.txt
            filename = basename
        
        # Alternatively, you could use model name + iteration + file number
        # filename = f"{model}_{basename}"
    else:
        filename = model
    ...
    # Save as TXT only
    print('Saving scores to ', f'{save_dir}/{filename}.txt')
    with open(f'{save_dir}/{filename}.txt', 'w') as f:
        f.write(f'alpha={round(Alpha_Precision_all,4)}\n')
        f.write(f'beta={round(Beta_Recall_all,4)}\n')
    
    print('Evaluation completed and saved as TXT file')
