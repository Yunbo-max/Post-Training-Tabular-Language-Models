...
if __name__ == '__main__':
    ...
    save_dir = f'eval/density/{dataname}'
    ...
    Quality = 1 - (Shape + Trend) / 2
    
    csv_path = os.path.split(syn_path)[1]
    csv_name = csv_path.split('.')[0]
    
    # NFEs
    # extracted_path = s_path.split("synthetic/", 1)[-1]
    # base_path = os.path.splitext(extracted_path)[0]
    # save_path = os.path.join("eval", "density", base_path + ".json")

    print('Saving scores to ', f'{save_dir}' + '.txt')
    print(f'cde={np.round(Shape, 4)}')  # Standard output for parsing
    print(f'pcc={np.round(Trend, 4)}')  # Standard output for parsing
    with open(f'{save_dir}' + '.txt', 'w') as f:
        f.write(f'cde={np.round(Shape, 4)}\n')
        f.write(f'pcc={np.round(Trend, 4)}')
