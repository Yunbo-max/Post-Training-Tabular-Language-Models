...
class GReaT:
    ...
    def __init__(self, llm: str, experiment_dir: str = "trainer_great", epochs: int = 100,
                 batch_size: int = 16, efficient_finetuning: str = "", **train_kwargs):
        """ Initializes GReaT.
    ...
    def fit(self, data: tp.Union[pd.DataFrame, np.ndarray], column_names: tp.Optional[tp.List[str]] = None,
        ...
        # transformers>=4.46 renamed `tokenizer` to `processing_class` in Trainer.__init__
        try:
            great_trainer = GReaTTrainer(self.model, training_args, train_dataset=great_ds,
    ...
    def sample(self, n_samples: int,
        ...
        # Stagnation guard: break the loop if `already_generated` does not increase
        # for `_no_progress_limit` consecutive attempts. Prevents an infinite spin
        # when the (post-training) model produces only malformed rows. Override via
        # GREAT_SAMPLE_NO_PROGRESS_K env (default 30 attempts ~= 5 minutes).
        _no_progress_limit = int(os.environ.get("GREAT_SAMPLE_NO_PROGRESS_K", "30"))
        _no_progress = 0
        # Wall-clock guard: cap the entire sample() call at GREAT_SAMPLE_MAX_SECONDS
        # (default 600s = 10 min). Catches the slow-but-not-zero pathology where
        # the model produces ~1 valid row per several seconds — without this guard
        # a single call could take 7+ hours and never trigger the no-progress break.
        import time as _time_mod
        _wall_start = _time_mod.time()
        _wall_limit = int(os.environ.get("GREAT_SAMPLE_MAX_SECONDS", "600"))
        with tqdm(total=n_samples) as pbar:
            ...
            while n_samples > df_gen.shape[0]:
                ...
                _new = df_gen.shape[0] - already_generated
                pbar.update(_new)
                if _new == 0:
                    _no_progress += 1
                    if _no_progress >= _no_progress_limit:
                        print(f"[great.sample] no progress for {_no_progress} attempts; "
                              f"breaking with {already_generated}/{n_samples} valid rows.")
                        break
                else:
                    _no_progress = 0
                ...
                # Wall-clock cap
                if _time_mod.time() - _wall_start > _wall_limit:
                    print(f"[great.sample] wall-clock cap {_wall_limit}s reached; "
                          f"breaking with {already_generated}/{n_samples} valid rows.")
                    break
