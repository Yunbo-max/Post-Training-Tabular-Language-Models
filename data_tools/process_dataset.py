...
def train_val_test_split(data_df, cat_columns, num_train = 0, num_test = 0):
    ...
    while True:
        ...
        #         print('set(train_df[i])',set(train_df[i]))
        #         print('set(data_df[i])',set(data_df[i]))
...
def process_data(name):
    ...
    if info['test_path']:
        # If testing data is given, combine train & test, then split 50-50
        ...
        for col in test_df:
            if test_df[col].dtype == 'object':
                test_df[col] = test_df[col].apply(lambda x: x.strip())

        train_df = data_df
        
       
    else:  
        # For all other cases, split 50-50
        data_df = data_df.reset_index(drop=True)  # Ensure clean indices
        num_train = int(num_data * 0.9)
        num_test = num_data - num_train

        train_df, test_df, seed = train_val_test_split(data_df, cat_columns, num_train, num_test)

    # NOW rename columns to numeric indices after the split
    ...
    # Replace missing marker '?' with proper missing values.
    # Use np.nan for numerical columns so dtype stays numeric, and pd.NA for categorical columns.
    # Convert categorical columns to object dtype first to avoid dtype compatibility warnings.
    for col in num_columns:
        train_df.loc[train_df[col] == '?', col] = np.nan
    for col in cat_columns:
        if train_df[col].dtype != 'object':
            train_df[col] = train_df[col].astype('object')
        train_df.loc[train_df[col] == '?', col] = pd.NA
    for col in num_columns:
        test_df.loc[test_df[col] == '?', col] = np.nan
    for col in cat_columns:
        if test_df[col].dtype != 'object':
            test_df[col] = test_df[col].astype('object')
        test_df.loc[test_df[col] == '?', col] = pd.NA
